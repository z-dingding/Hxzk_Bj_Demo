...
public class PatternLockView extends View {
    ...
    public @interface PatternViewMode {
         ...
         * 此状态表示用户正确绘制的模式。路径的颜色和两个点都会变成这个颜色。
         * <p>
         ...
         * the dots both would be changed to this color.
         * 此状态表示用户错误绘制的模式。路径的颜色和两个点都会变成这个颜色。
         * <p>
   ...
   // onMeasure() 在View放置到父容器时调用
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!mAspectRatioEnabled) {
         ...
         //getSuggestedMinimumWidth() 获取宽度的最小值
        ...
        //setMeasureDimension()函数的作用——这个函数调用中传进去的值是View最终的视图大小
...
//                查找表(只有在这种情况下才不同动画)
                ...
                if (i != 0) {
                    ...
                    currentPath.rewind();
                    //moveTo 不会进行绘制，只用于移动画笔,需要结合lineTo使用。lineTo 用于进行直线绘制，默认从坐标(0,0)开始绘制。
    ...
    /**
     * 计算获取合适的宽高
     * 是为了根据ViewGroup中具体能够提供的空间大小来指定子View的视图大小
     * @param measureSpec
     * @param desired
     * @return
     */
...
//        在ViewGroup中，给View分配的空间大小并不是确定的，有可能随着具体的变化而变化，
//        而这个变化的条件就是传到specMode中决定的，specMode一共有三种可能
        switch (MeasureSpec.getMode(measureSpec)) {
            ...
            case MeasureSpec.UNSPECIFIED:
                result = desired;
                break;
           //MeasureSpec.AT_MOST：子视图的大小最多是specSize中指定的值
